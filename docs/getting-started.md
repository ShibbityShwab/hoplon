...
3. Unsets host `OPENCODE_*` selectors.
...
The host filesystem is hidden except the repo and the target directory. Network
stays up for the Venice API. Raw-socket tooling does not work inside; run that
unsandboxed. See [Sandbox](sandbox.md).
